...
var Databases = React.createClass({
    ...
    tree: function (databases){
      console.log(databases)
      root = { title: "root", databases: [], children: [] };
      node_map = { root: root }
      _.each(this.props.hierarchy,
             function (n){
                 parent_id = n.parent_id || 'root'
                 n.children = node_map.hasOwnProperty(n.id) ? node_map[n.id].children.slice(0) : []
                 node_map[n.id] = n 
                 var re = new RegExp(n.title,"g");
                 dbs = _.select(databases,
                                      function (db) {
                                          return db.name.match(re);
                        });
                 if (dbs.length > 0) n.databases = _.map(dbs,function(db) { return { id: db.id, type: db.type, title: db.title }  });
                 if (!node_map.hasOwnProperty(parent_id)){
              ...
              className="form-group databases-container">
                {
                    _.map(this.categories(), _.bind(function (category) {
                        console.log(this.tree(this.databases(category)))
                        return (
                            <div
        ...
        return (
            <div
                ...
                <form
                    ...
                    <Databases ref="databases" onDatabaseTypeChanged={this.handleDatabaseTypeChanaged} databases={this.state.databases} hierarchy={this.state.hierarchy}/>
                    <div
                        className="form-group">
                        <Options ref="opts"/>
